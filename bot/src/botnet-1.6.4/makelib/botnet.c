/* Bot Net Main file
  (c) Christophe CALMEJANE - 1999'03
...
struct BN_threadspecific {
  ...
  char **BN_StatsList;
  char *BN_LinksServer;
  int BN_ChanCount,BN_TopicCount,BN_ChanUsersCount,BN_WhoisCount,BN_NameCount,BN_WhoCount,BN_BanCount,BN_LinksCount,BN_StatsCount;
#ifdef SERVER
  char **BN_NJoinList;
  int BN_NJoinCount;
#endif
...
void BN_destroyts(void *ptr)
{
  ...
  BN_FreeList(ts->BN_LinksList,ts->BN_LinksCount);
  BN_FreeWhoisInfos();
...
void BN_HandleMessage(BN_PInfo I,BN_PMessage Msg)
{
  ...
  else if(atoi(Msg->Command) != 0 )
  {
    ...
    switch(Code)
    {
      ...
      case RPL_WHOISREALIP   : BN_AddInfoToWhois(Msg->Params[1],Msg->Params[2],WHOIS_REALIP); break;
      ...
      case RPL_VERSION : if(I->CB.OnVersion != NULL) I->CB.OnVersion(I,Msg->Params[2],Msg->Params[1],Msg->Params[3]);break;
      case RPL_LUSERLOCAL : if(I->CB.OnLocalUsers != NULL) I->CB.OnLocalUsers(I,Msg->Prefix,Msg->Params[1]);break;
      case RPL_LUSERGLOBAL : if(I->CB.OnGlobalUsers != NULL) I->CB.OnGlobalUsers(I,Msg->Prefix,Msg->Params[1]);break;
      case 2:
      case 3:
      case 4:
      ...
      case RPL_LINKS : if(ts->BN_LinksServer == NULL) ts->BN_LinksServer = strdup(Msg->Prefix);BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_LinksList,Tmp,&ts->BN_LinksCount); break;
      case RPL_ENDOFLINKS : if(I->CB.OnLinks != NULL) I->CB.OnLinks(I,ts->BN_LinksServer,(const char **)ts->BN_LinksList,ts->BN_LinksCount);BN_FreeList(ts->BN_LinksList,ts->BN_LinksCount);ts->BN_LinksCount=0;if(ts->BN_LinksServer != NULL) free(ts->BN_LinksServer);break;
      case RPL_STATSCLINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSNLINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSILINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSYLINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSULINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSKLINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSLLINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSUPTIME : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSOLINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_STATSHLINE : BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));BN_AddStringToList(&ts->BN_StatsList,Tmp,&ts->BN_StatsCount); break;
      case RPL_ENDOFSTATS : if(I->CB.OnStats != NULL) I->CB.OnStats(I,Msg->Prefix,(const char **)ts->BN_StatsList,ts->BN_StatsCount);BN_FreeList(ts->BN_StatsList,ts->BN_StatsCount);ts->BN_StatsCount=0;break;
  ...
  else if(STRICMP_FUNC(Msg->Command,"NICK") == 0)
  {
    ...
    if(I->CB.OnNick != NULL)
      I->CB.OnNick(I,Msg->Prefix,Msg->Params[0]);
#ifdef SERVER
    }
#endif /* SERVER */
  }
  ...
  {
    BN_CreateStatusLine(Msg->Params,1,Msg->NbParams-1,Tmp,sizeof(Tmp));
    if(strcmp(Msg->Params[0],I->Nick) == 0) /* Me ? */
    {
      if(I->CB.OnKilled != NULL)
        I->CB.OnKilled(I,Msg->Prefix,Tmp);
    }
    else
    {
      if(I->CB.OnKill != NULL)
  ...
  else if(STRICMP_FUNC(Msg->Command,"SJOIN") == 0)
  {
    if(Msg->NbParams == 5)
    {
      ...
      if(I->SCB.OnSJoin != NULL)
        I->SCB.OnSJoin(I,Msg->Params[2],atol(Msg->Params[0]),atol(Msg->Params[1]),Msg->Params[3],NULL,NULL,(const char **)ts->BN_NJoinList,ts->BN_NJoinCount);
      BN_FreeList(ts->BN_NJoinList,ts->BN_NJoinCount);
    }
    else if(Msg->NbParams == 6)
    {
      ...
      if(I->SCB.OnSJoin != NULL)
        I->SCB.OnSJoin(I,Msg->Params[2],atol(Msg->Params[0]),atol(Msg->Params[1]),Msg->Params[3],Msg->Params[4],NULL,(const char **)ts->BN_NJoinList,ts->BN_NJoinCount);
      BN_FreeList(ts->BN_NJoinList,ts->BN_NJoinCount);
    }
    else if(Msg->NbParams == 7)
    {
      context;
      ts->BN_NJoinCount = 0;
      BN_ParseStringList(Msg->Params[6],&ts->BN_NJoinList,&ts->BN_NJoinCount," ",ts->tmp_tok);
      if(I->SCB.OnSJoin != NULL)
        I->SCB.OnSJoin(I,Msg->Params[2],atol(Msg->Params[0]),atol(Msg->Params[1]),Msg->Params[3],Msg->Params[4],Msg->Params[5],(const char **)ts->BN_NJoinList,ts->BN_NJoinCount);
      BN_FreeList(ts->BN_NJoinList,ts->BN_NJoinCount);
    }
