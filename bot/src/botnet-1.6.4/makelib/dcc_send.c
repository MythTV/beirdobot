...
void BN_SendDCCSendRequest(BN_PInfo I,const char *Nick,const int Flags, char *Filename, const int Timeout,void *User)
{
  BN_PTempoStruct TS;
#ifdef _WIN32
  DWORD son;
      ...
      if(sent < Send->Length)
      {
        ...
        do
        {
          ...
          if(len <= 0)
          {
            ...
            if(Send->CB.OnDCCSendClosed != NULL)
              Send->CB.OnDCCSendClosed(I,Send,DCC_FAILED);
            BN_PrintDebug(2,"DCC send failed (read error)\n");
            BN_CloseDCCSend(I, Send);
            return;
          }
          received = ((unsigned char)(Buffer[0]) * 256 * 256 * 256) + ((unsigned char)(Buffer[1]) * 256 * 256) + ((unsigned char)(Buffer[2]) * 256) + (unsigned char)(Buffer[3]);
        } while(received < sent);
      }
    }
    fclose(File);
